...
import { useAvailability } from "./context/AvailabilityContext"; // ✅ IMPORTED

export default function DeepCleaning() {
  const router = useRouter();

  const [packageType, setPackageType] = useState<"Standard" | "Premium" | "Deluxe">("Standard");
  const [gender, setGender] = useState<"Male" | "Female">("Male");

  const prices = {
    Standard: { price: 8000, hours: "4 Hours", contract: "Session" },
    Premium: { price: 12000, hours: "6 Hours", contract: "Session" },
    Deluxe: { price: 18000, hours: "8 Hours", contract: "Session" },
  };

  // ✅ USE GLOBAL CONTEXT TO GET LIVE AVAILABILITY DATA
  ...
  const handleBook = () => {
    const selectedPrice = prices[packageType].price;
    router.push({
      pathname: "/booking",
      params: {
        serviceType: "Deep Cleaning",
        packageType: packageType,
        hours: prices[packageType].hours,
        contract: prices[packageType].contract,
        price: selectedPrice,
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>✨ Deep Cleaning</Text>
      <Text style={styles.subtitle}>Select your deep clean package</Text>

      {(["Standard", "Premium", "Deluxe"] as const).map((pkg) => (
        <TouchableOpacity
          key={pkg}
          style={[styles.card, packageType === pkg && styles.activeCard]}
          onPress={() => setPackageType(pkg)}
        >
          <Text style={styles.planTitle}>{pkg}</Text>
          <Text style={styles.planDesc}>
            {pkg === "Standard" && "Kitchen + Bathroom deep scrub"}
            {pkg === "Premium" && "Full home + Sofa + Carpet cleaning"}
            {pkg === "Deluxe" && "Complete sanitization + Pest control"}
          </Text>
          <View style={styles.planMeta}>
            <Text style={styles.planPrice}>Rs {prices[pkg].price.toLocaleString()}</Text>
            <Text style={styles.planHours}>{prices[pkg].hours} / {prices[pkg].contract}</Text>
          </View>
        </TouchableOpacity>
      ))}
      ...
      <TouchableOpacity style={styles.button} onPress={handleBook}>
        <Text style={styles.btnText}>Book Deep Cleaning</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
